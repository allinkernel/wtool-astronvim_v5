...
fetch() {
    ...
    while :; do
        ...
        # 环境里有代理变量、而代理恰好连不上 GitHub 时，干等没有意义：
        # 第一次失败之后就绕开代理再试一次。
        #
        # **别把它读成"代理对 GitHub 是坏的"。** 我一度这么写过，那是错的 ——
        # 当时量到一次 SSL 断连就下了结论，后来复测代理完全正常
        # （api.github.com 200/1.1s、github.com 200/2.8s）。
        # 真实情况是：代理**偶尔**会抖，而抖的时候表现是"慢慢磨"不是立刻报错。
        # 所以这里的逻辑是"两条路都试过才算失败"，不是"代理不能用"。
        if [ "$_nop" = 0 ] \
...
if [ ! -s "$CACHE/.todo.tsv" ]; then
    ...
    _launch() {
        ...
        # 取 $! 前临时关掉 set -u。
        # set -u 下引用一个未设置的 $! 会让**整个脚本**立刻退出，
        # 表现就是"16 个卷全下完了、清单也写好了，却报 download 失败"——
        # 最气人的失败模式：活干完了，报错说没干成。
        # 这个 pid 只用来收割和显示，取不到就算了，不值得为它中断整个下载。
        set +u
        _pid=$!
        set -u
        if [ -z "${_pid:-}" ]; then
            warn "  $_n 起了后台任务但取不到 pid，不跟踪它（它会自己跑完）"
            continue
        fi
