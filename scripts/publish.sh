...
say "薅出来 $_n 项"
# 这个检查的目的是"看有没有整个薅漏"，不是"必须精确多少 G"。
# 原来写的是"应该 1G 往上"，那是 75 个 mason 包时代的数字；
# 清单减到 5 个之后 576M 是正常的，它却还在喊"体积偏小"——
# 假警报和漏报一样坏，会训练人忽略告警。
# 真薅漏的典型症状是只剩几百 K（软链/空目录），所以门槛放在 50M。
_du=$(du -sh "$WORK/payload/home" 2>/dev/null | cut -f1)
say "产物体积 : $_du"
case $_du in
    [0-9]*K) warn "体积只有 $_du，多半整个薅漏了，检查上面的清单" ;;
    [0-9]M|[1-4][0-9]M) warn "体积只有 $_du，偏小，确认一下清单里的东西是否都在" ;;
esac
